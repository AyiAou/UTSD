...
def main(cfg):
    ...
    model, _, _ = model_instance(
        ...
        in_len=cfg.data.in_len, out_len=cfg.data.out_len, type='sampling'
    )
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            lookback, _ = batch
            lookback = lookback.to(dtype=torch.float32).to(device)
            lookback = lookback.permute(0, 2, 1)
            model.updateReferenceAttentionControl(
                guidance_scale=cfg.model.guidance_scale,
                batch_size=lookback.shape[0]
            )
            fake_image = model.sampling(
                gen_shape=lookback.shape,
            ...
            true_image = lookback
            fake_image_.append(fake_image.detach().cpu())
            true_image_.append(true_image.detach().cpu())
            print('Step:{};'.format(batch_idx))

    fake_image_ = np.concatenate(fake_image_, axis=0)
    true_image_ = np.concatenate(true_image_, axis=0)
    np.save(file=f"{save_dir}/validation/valid_fake.npy", arr=fake_image_)
    np.save(file=f"{save_dir}/validation/valid_true.npy", arr=true_image_)
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(cfg.output_dir, project_name, 'inference_sampling')
    os.makedirs(save_dir, exist_ok=True)
    if os.path.exists(f'./{save_dir}/inference_stage_4.txt'):
        os.remove(f'./{save_dir}/inference_stage_4.txt')
    sys.stdout = Logger(f'./{save_dir}/inference_stage_4.txt')
