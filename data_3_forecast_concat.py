...
class Dataset_ETT_hour(Dataset):
    def __init__(self, root_path, data_path, flag, size, downSampleScale=1, few_shot=1.0, large_shot=40,
    ...
    def __getitem__(self, index):
        index = index // self.large_shot
        idx_channel = index % self.data.shape[1]
        idx = index // self.data.shape[1]
        # Original
        s_begin = idx
        s_end = s_begin + self.seq_len
        r_begin = s_end - self.label_len
        r_end = r_begin + self.label_len + self.pred_len
        seq_x = self.data[s_begin:s_end, idx_channel]
        seq_y = self.data[r_begin:r_end, idx_channel]
        seq_x = np.expand_dims(seq_x, axis=1)
        seq_y = np.expand_dims(seq_y, axis=1)
        return seq_x, seq_y

    def __len__(self):
        return self.all_size * self.data.shape[1] * self.large_shot       # 【改】
...
class Dataset_ETT_minute(Dataset):
    def __init__(self, root_path, data_path, flag, size, downSampleScale=1, few_shot=1.0, large_shot=20,
    ...
    def __len__(self):
        return self.all_size * self.data.shape[1] * self.large_shot
...
class Dataset_Custom(Dataset):
    def __init__(self, root_path, data_path, flag, size, downSampleScale=1, few_shot=1.0, large_shot=1,
                 features='M', target='OT', scale=True, timeenc=0, freq='h'):
...
class Dataset_TSF_longUni(Dataset):
    def __init__(self, root_path, data_path, size, flag='train', downSampleScale=1.0, few_shot=1.0, step=1.0, large_shot=1):
        self.seq_len = size[0]
        self.label_len = size[1]
        self.pred_len = size[2]
        self.flag = flag
...
def getTimeSeriesDataset(data, input_len, pred_len, flag, down_scale=1.0, step=4.0):
    size = [input_len, 0, pred_len]

    if data in ['ETTh1', 'ETTh2']:
        root_path = './dataset/ETT-small/'
        data_path = '{}.csv'.format(data)
        dataset = Dataset_ETT_hour(root_path=root_path, data_path=data_path, flag=flag, size=size, large_shot=40)
    elif data in ['ETTm1', 'ETTm2']:
        root_path = './dataset/ETT-small/'
        data_path = '{}.csv'.format(data)
        dataset = Dataset_ETT_minute(root_path=root_path, data_path=data_path, flag=flag, size=size, large_shot=20)
    elif data in ['ECL']:
        root_path = './dataset/electricity/'
        data_path = 'electricity.csv'
        dataset = Dataset_Custom(root_path=root_path, data_path=data_path, flag=flag, size=size, large_shot=1)
        ...
        root_path = './dataset/exchange_rate/'
        data_path = 'exchange_rate.csv'
        dataset = Dataset_Custom(root_path=root_path, data_path=data_path, flag=flag, size=size, large_shot=30)
    elif data in ['Weather']:
        root_path = './dataset/weather/'
        data_path = 'weather.csv'
        dataset = Dataset_Custom(root_path=root_path, data_path=data_path, flag=flag, size=size, large_shot=5)
    elif data in ['Sunspot']:
        root_path = './dataset'
        data_path = 'Sunspot.tsf'
        dataset = Dataset_TSF_longUni(root_path=root_path, data_path=data_path, size=size, flag=flag, downSampleScale=down_scale, step=step, large_shot=500)
    elif data in ['RiverFlow']:
        root_path = './dataset'
        data_path = 'SaugeenRiverFlow.tsf'
        dataset = Dataset_TSF_longUni(root_path=root_path, data_path=data_path, size=size, flag=flag, downSampleScale=down_scale, step=step, large_shot=2000)
    elif data in ['USBirths']:
        root_path = './dataset'
        data_path = 'USBirths.tsf'
        dataset = Dataset_TSF_longUni(root_path=root_path, data_path=data_path, size=size, flag=flag, downSampleScale=down_scale, step=step, large_shot=5000)
    elif data in ['SolarPower']:
        root_path = './dataset'
        data_path = 'SolarPower.tsf'
        dataset = Dataset_TSF_longUni(root_path=root_path, data_path=data_path, size=size, flag=flag, downSampleScale=down_scale, step=step, large_shot=1)
...
def getConcatDatsset(input_len, pred_len, flag):
    """                                                                                 27,593,879
    DataName:ETTh1 DataSize:(8640, 7) SampleNum:55783           # 20        100w        1,115,660
    DataName:ETTh2 DataSize:(8640, 7) SampleNum:55783           # 20        100w        1,115,660
    DataName:ETTm1 DataSize:(34560, 7) SampleNum:237223         # 10        200w        2,372,230
    DataName:ETTm2 DataSize:(34560, 7) SampleNum:237223         # 10        200w        2,372,230
    DataName:Weather DataSize:(36887, 21) SampleNum:760536      # 5         350w        3,802,680
    DataName:ECL DataSize:(18412, 321) SampleNum:5694861        # 1         500w        5,694,861
    DataName:Traffic DataSize:(12280, 862) SampleNum:10006958   # 1         1000w       10,006,958
    DataName:Exchange DataSize:(5311, 8) SampleNum:37120        # 30        100w        1,113,600

    DataName:Sunspot DataSize:(51746, 1) SampleNum:1065         # 500       50w
    DataName:RiverFlow DataSize:(16618, 1) SampleNum:333        # 2000      50w
    DataName:USBirths DataSize:(5113, 1) SampleNum:93           # 5000      50w
    DataName:SolarPower DataSize:(5178055, 1) SampleNum:1294346 # 1         100w
    DataName:WindPower DataSize:(5178002, 1) SampleNum:1294333  # 1         100w
    19692033
    19692033
    19692033
    DataName:ETTh1 DataSizePercent:0.0028327699836781708
    DataName:ETTh2 DataSizePercent:0.0028327699836781708
    DataName:ETTm1 DataSizePercent:0.012046648510085271
    DataName:ETTm2 DataSizePercent:0.012046648510085271
    DataName:Exchange DataSizePercent:0.0018850262946441335
    DataName:Weather DataSizePercent:0.03862150748985643
    DataName:ECL DataSizePercent:0.2891961942172248
    DataName:Traffic DataSizePercent:0.5081729245527874
    DataName:Sunspot DataSizePercent:0.0006484348264092387
    DataName:RiverFlow DataSizePercent:0.00020246766801579094
    DataName:USBirths DataSizePercent:5.641875574756553e-05
    DataName:SolarPower DataSizePercent:0.06572942468662327
    DataName:WindPower DataSizePercent:0.06572876452116447
    """
    ...
    print('')
    data_list = [
        'ETTh1', 'ETTh2', 'ETTm1', 'ETTm2', 'Exchange', 'Weather', 'ECL', 'Traffic',
        'Sunspot', 'RiverFlow', 'USBirths', 'SolarPower', 'WindPower'
    ]
    dataset_list = []
    batch_num_list = []
    for data in data_list:
        dataset = getTimeSeriesDataset(data=data, input_len=input_len, pred_len=pred_len, flag=flag)
        dataset_list.append(dataset)
        print(f'DataName:{data} DataSize:{dataset.data.shape} SampleNum:{len(dataset)}')
        batch_num_list.append(len(dataset))

    data_set_all = ConcatDataset(dataset_list)
    print(f'SampleNum of ALL_CONCAT is:')
    print(sum(batch_num_list))
    print(len(data_set_all))
    for i in range(len(data_list)):
        print(f'DataName:{data_list[i]} DataSizePercent:{batch_num_list[i]/sum(batch_num_list)}')

    return data_set_all


def show_ecl_traffic(data):
    def my_show_1d(array_1d, save_dir, pic_name):
        import matplotlib.pyplot as plt
        array_1d = list(array_1d)
        x = list(range(len(array_1d)))
        plt.figure(num=3, figsize=(40, 4))
        plt.plot(x, array_1d, color='red', linewidth=1)
        plt.savefig(f'{save_dir}/{pic_name}.png')
        plt.show()
        plt.close()

    dataset = getTimeSeriesDataset(data=data, input_len=336, pred_len=336, flag='train')
    print(f'DataName:{data} DataSize:{dataset.data.shape}')

    save_dir = f'./show_{data}'
    os.makedirs(name=save_dir, exist_ok=True)
    for channel in range(dataset.data.shape[1]):
        my_show_1d(
            array_1d=dataset.data[:, channel],
            save_dir=save_dir,
            pic_name=str(channel)
        )
...
if __name__ == '__main__':
    ...
    getConcatDatsset(input_len=336, pred_len=336, flag='train')
    # getConcatDatsset(input_len=336, pred_len=336, flag='test')
    # getConcatDatsset(input_len=336, pred_len=336, flag='val')

    # show_ecl_traffic('ECL')
    # show_ecl_traffic('Traffic')
