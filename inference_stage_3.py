...
def main(cfg):
    ...
    np.save(file=f"{save_dir}/validation/valid_look.npy", arr=look_image_)
    np.save(file=f"{save_dir}/validation/valid_pred.npy", arr=pred_image_)
    ...
    f.write(f'Inference_Stage_3: {args.mode} in {args.config}' + '\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--is_show", action="store_true", default=False)
    ...
    save_dir = os.path.join(cfg.output_dir, project_name, 'inference_forecasting')
